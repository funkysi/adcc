<?php
 ...
 <title>Arnold &amp; District Camera Club: Schedule</title>
 <link>'.$url.'schedule/</link>
...
//Sun, 06 Jul 2008 18:35:19 +0100
	...
	$num_rows = mysql_num_rows(mysql_query("select * from schedule order by date desc"));     
...
$orgdate=date("D, d M Y H:i:s O", mktime($hour,$min,$sec,$month,$day,$year));
	    ...
	    '  
	        <item>   
	        <title>'.str_replace('&','&amp;',mysql_result($result,$i,"event")).'</title>   
	        <link>'.$url_product.'</link>   
	        <description>'.str_replace('&','&amp;',mysql_result($result,$i,"event"))." - ".htmlspecialchars(strip_tags(mysql_result($result,$i,"misc"))).'</description>   
